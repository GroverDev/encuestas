...
{
    public static async Task<IEnumerable<Permiso>> ObtenerPermisos()
    {
        var db = new DapperDb(DB.GestorDB.POSTGRESQL, DB.Sistema.ENCUESTA, DB.BaseDeDatos.LOCAL);
        try
        {
            db.CreateConnection.Open();
            const string sql = "SELECT id, codigo, nombre FROM permiso ORDER BY codigo";
            return await db.CreateConnection.QueryAsync<Permiso>(sql);
        }
    ...
    public static async Task<Permiso?> ObtenerPermisoPorId(Guid id)
    {
        var db = new DapperDb(DB.GestorDB.POSTGRESQL, DB.Sistema.ENCUESTA, DB.BaseDeDatos.LOCAL);
        try
        {
            db.CreateConnection.Open();
            const string sql = "SELECT id, codigo, nombre FROM permiso WHERE id = @Id";
            return await db.CreateConnection.QueryFirstOrDefaultAsync<Permiso>(sql, new { Id = id });
        }
        catch (NpgsqlException ex) when (ex.InnerException is SocketException) { throw new ExceptionControlado("El servidor de la base de datos está caído o inaccesible."); }
        catch (NpgsqlException ex) { throw new ExceptionControlado($"Error al interactuar con la base de datos: {ex.Message}"); }
        catch (ExceptionControlado ex) { throw new ExceptionControlado(ex.Message, ex); }
    ...
    public static async Task<bool> CrearPermiso(PermisoRequest request)
    {
        var db = new DapperDb(DB.GestorDB.POSTGRESQL, DB.Sistema.ENCUESTA, DB.BaseDeDatos.LOCAL);
        try
        {
            ...
            try
            {
                const string sql = "INSERT INTO permiso (codigo, nombre) VALUES (@Codigo, @Nombre)";
    ...
    public static async Task<bool> ActualizarPermiso(PermisoRequest request)
    {
        var db = new DapperDb(DB.GestorDB.POSTGRESQL, DB.Sistema.ENCUESTA, DB.BaseDeDatos.LOCAL);
        try
        {
            ...
            try
            {
                const string sql = "UPDATE permiso SET codigo = @Codigo, nombre = @Nombre WHERE id = @Id";
                await db.CreateConnection.ExecuteAsync(sql, request, transaction: transaction);
                transaction.Commit();
                return true;
            }
    ...
    public static async Task<bool> EliminarPermiso(Guid id)
    {
        var db = new DapperDb(DB.GestorDB.POSTGRESQL, DB.Sistema.ENCUESTA, DB.BaseDeDatos.LOCAL);
        try
        {
            ...
            try
            {
                const string sql = "DELETE FROM permiso WHERE id = @Id";
